...
IT IS NOT EVEN USED BY THE PROJECT 
##################################*/
...
function App(){
	...
	/*##################################	
	NEXT TODO:
	We need to REPLACE states to useReducer
	to simplify all this complex many-state 
	login action

state=			init 		get-token 		get-url		
myToken			is checked		
auth-url		is null
response=		is null
profile-url=	is null
...
Explanation
the above is supposed to represent a table.
	##################################*/
	...
	/*##################################	
	Another fetch to obtain the profileData
	...
	if (myToken){
		...
		profileFetch.then(data=>{
			setProfileData(data)})
			// data looks like {username:... first_name:.... , email:... etc}
		}
		}
